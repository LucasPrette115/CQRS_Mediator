...
{
    public class CreateMemberCommandHandler(IMemberRepository memberRepository, IUnitOfWork unitOfWork) : IHandler<CreateMemberCommand, Result<CreateMemberResponse>>
    {
        ...
        public async Task<Result<CreateMemberResponse>> HandleAsync(CreateMemberCommand request, CancellationToken cancellation = default)
        {
            try
            {
                var member = new Domain.Entities.Member(request.FirstName, request.LastName, request.Email, request.Gender, request.IsActive);
